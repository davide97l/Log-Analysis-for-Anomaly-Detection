"""
The interface for log_data preprocessing.
Authors:
    LogPAI Team
"""
...
class FeatureExtractor(object):
    ...
    def fit_transform(self, X_seq, term_weighting=None, normalization=None, oov=False, min_count=1):
        """ Fit and transform the log_data matrix
        Arguments
        ---------
        ...
        Returns
        -------
            X_new: The transformed log_data matrix
        """
        print('====== Transformed train log_data summary ======')
        ...
        print('Train log_data shape: {}-by-{}\n'.format(X_new.shape[0], X_new.shape[1]))
        return X_new

    def transform(self, X_seq):
        """ Transform the log_data matrix with trained parameters
        Arguments
        ---------
        ...
        -------
            X_new: The transformed log_data matrix
        """
        print('====== Transformed test log_data summary ======')
        X_counts = []
        for i in range(X_seq.shape[0]):
            event_counts = Counter(X_seq[i])
            X_counts.append(event_counts)
